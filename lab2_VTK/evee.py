...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        central = np.sum(self.nodes, axis=1)/self.nodes.shape[1]
        e = self.nodes.copy()
        e[0, :] -= central[0]
        e[1, :] -= central[1]
        e[2, :] -= central[2]
        e[0, :] /= np.sqrt(np.sum(self.nodes**2, axis=0))
        e[1, :] /= np.sqrt(np.sum(self.nodes**2, axis=0))
        e[2, :] /= np.sqrt(np.sum(self.nodes**2, axis=0))
        # print(e.shape)
        self.velocity[0] = e[0] * 10 * np.sin(self.nodes[0]) 
        self.velocity[1] = e[1] * 10 * np.sin(self.nodes[1])
        self.velocity[2] = e[2] * 10 * np.sin(self.nodes[2])
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("eevee-" + str(snap_number) + ".vtu")
        writer.Write()
...
try:
    ...
    gmsh.merge(os.path.join(path, 'eevee.stl'))
except:
