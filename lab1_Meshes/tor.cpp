...
int main(int argc, char **argv)
{
  ...
  // directory):
  try {
    gmsh::merge("./TorEmpty.stl");
    // gmsh::merge("./examples/t13_data.stl");
  } catch(...) {
  ...
  if(funny)
    gmsh::model::mesh::field::setString(f, "F", "(1e-1)*Sin((x+y)/5) +1.5e-1");
  else
    gmsh::model::mesh::field::setString(f, "F", "1e-1");
